...
const EMPTY_FORM = { startDate: "", endDate: "", type: "Exam", title: "", batches: "", details: "" };
...
export default function AdminDashboard() {
  ...
  const editEntry = (entry) => {
    ...
    setForm({ startDate: entry.startDate, endDate: entry.endDate, type: entry.type, title: entry.title, batches: entry.batches || "", details: entry.details || "" });
    setMessage(""); setError("");
    ...
    <main className="page admin-page">
      ...
      <section className="admin-card">
        ...
        <form className="admin-form" onSubmit={submit}>
          ...
          <div className="field"><label htmlFor="type">Type</label><input id="type" list="type-options" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} required /><datalist id="type-options"><option value="Exam" /><option value="Event" /><option value="Sport" /><option value="HPL" /><option value="Holiday" /></datalist></div>
          <div className="field"><label htmlFor="batches">Applicable batches / years</label><input id="batches" type="text" placeholder="e.g. 2026, 2025" value={form.batches} onChange={(e) => setForm({ ...form, batches: e.target.value })} /><span className="hint">Enter the admission batches that have the paper.</span></div>
      ...
      <section className="admin-card">
        ...
        <p className="hint">CSV columns: <code>Start Date, End Date, Type, Title, Batches / Years, Details</code>. For a single-day item, Start Date and End Date can be the same.</p>
      </section>
