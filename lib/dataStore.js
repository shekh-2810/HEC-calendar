...
export async function exportCsv() {
  const entries = await getEntries();
  const header = ["Start Date", "End Date", "Type", "Title", "Details"];
  const rows = [header, ...entries.map((e) => [e.startDate, e.endDate, e.type, e.title, e.details])];
...
export function normalizeEntry(entry) {
  ...
  return { startDate, endDate, type, title, details };
}
