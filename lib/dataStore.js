...
const BLOB_PATH = "hec/calendar.json";
...
async function ensureLocalStore() {
  ...
  try {
    ...
    await writeLocalEntries(DEFAULT_DATA);
  }
}
...
async function writeBlobEntries(entries) {
  await put(
    BLOB_PATH,
    JSON.stringify(entries, null, 2) + "\n",
    {
...
async function writeEntries(entries) {
  if (isVercelProduction()) {
    await writeBlobEntries(entries);
    return;
  }
...
export function parseCsv(text) {
  ...
  const idx = {
    ...
    startDate: header.findIndex(
      (h) => h === "start date" || h === "startdate"
    ),
    endDate: header.findIndex(
      (h) => h === "end date" || h === "enddate"
    ),
    type: header.findIndex(
      (h) => h === "type" || h.startsWith("type")
    ),
    title: header.findIndex(
      (h) => h === "title" || h.startsWith("title")
    ),
    batches: header.findIndex(
      (h) =>
        h === "batches / years" ||
        h === "batches" ||
        h === "batch" ||
    ...
    details: header.findIndex(
      (h) => h === "details" || h.startsWith("detail")
    ),
  };

  if (
    (idx.date < 0 && idx.startDate < 0) ||
    idx.type < 0 ||
    idx.title < 0
  ) {
    throw new Error(
      "CSV needs Date OR Start Date/End Date, plus Type, Title and optional Details."
    );
  }
  ...
  return useful.slice(1).map((r) => ({
    ...
    startDate:
      idx.startDate >= 0 ? r[idx.startDate] || "" : "",
    endDate:
      idx.endDate >= 0 ? r[idx.endDate] || "" : "",
    type: idx.type >= 0 ? r[idx.type] || "" : "",
    title: idx.title >= 0 ? r[idx.title] || "" : "",
    batches:
      idx.batches >= 0 ? r[idx.batches] || "" : "",
    details:
      idx.details >= 0 ? r[idx.details] || "" : "",
  }));
}
